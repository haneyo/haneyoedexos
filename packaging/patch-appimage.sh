...
// #58/#50:#36 部署态(#36 已部署,类体 = 本常量原文,浏览器仍是 browsh --startup-url)→ 桥接锚点,
// revert 阶段先换成新类体(browsh 位置参数 URL 修复 + cover session 方法),与 pristine apply 后的
// CLI_PANEL_NEW 一致。锚点保持原文不动 —— 它描述的是 #36 部署态真实内容。
...
// ---- #50 Part B:CliPanel cover session 方法(browsh 版类体扩展)----
// 镜像 src cliPanel.class.js 的 beginCoverSession/coverTerm/endCoverSession(压缩版)。
// 从 CLI_PANEL_CLASS_NEW_OLD 派生(浏览器相关差异见上面 CLI_PANEL_CLASS_NEW,cover 方法
// 与浏览器无关),类尾 toggleDevTools(){}} 前注入三个 cover 方法(临时 `__cover__` 会话,
// 真 pty `sh -c "stty raw -echo; exec cat"`,Terminal wrapper muted:true)。
// derive 保证新类体与 #36 部署态旧类体逐字兼容,桥接零漂移。
...
const CLI_PANEL_CLASS_NEW = CLI_PANEL_CLASS_NEW_OLD
  ...
  // #162 (reverses #58): browsh's startup URL is a POSITIONAL argument — there is
  // NO --startup-url flag (pflag rejects it and the browser dies on launch; that
  // non-existent flag is why the old browsh was unusable). #136:start page =
  // local dark search page (search bar + switchable engine), installed next to
  // the AppImage at /opt/edex/cli-start.html (see build-iso.sh). browsh renders
  // through the baked headless Firefox at /opt/firefox.
  .join('{id:"browsh",name:"browsh",cmd:["browsh","file:///opt/edex/cli-start.html"],icon:"browser"}')
...
const targets = [
  {
    ...
    transform: c => c
      ...
      // browsh/浏览器子进程不杀,视频声音驻留)。锚点用 pristine 的 minified 形态(变量 d,extraTtys 条目);
  ...
  // #162 (reverses #58):carbonyl SGR y+1 补偿目标已删除 —— 其 expectOut='_sgrMouseFix'
  // 会让不含该标志的新 browsh 构建误注入 carbonyl 专属代码。browsh 走 Firefox Marionette,
  // 鼠标事件由 tcell 处理,与终端 SGR 鼠标协议无关。原 #136 SGR 修复对 browsh 无意义。
  {
  ...
  },
  {
    ...
    // #50/#58:标记换成 beginCoverSession(#50 cover 方法 / 浏览器类体的特征串)。已打 #36 的部署版
    ...
    transform: c => c
      ...
      // #50/#58:#36 部署态(类体 = CLI_PANEL_CLASS_NEW_OLD,浏览器 = browsh --startup-url、无 cover 方法)先桥接成
