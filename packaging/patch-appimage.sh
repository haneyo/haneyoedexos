...
const ZH_SSH = "\"settings.cat.ssh\":\"SSH 远程登录\",\"settings.ssh.status\":\"服务状态\",\"settings.ssh.enabled\":\"SSH 服务\",\"settings.ssh.enabled.help\":\"启用/停用 OpenSSH 服务器(sshd)。需要从其它设备远程连到本机时保持开启。\",\"settings.ssh.running\":\"运行中\",\"settings.ssh.stopped\":\"已停止\",";
...
const EN_SSH = "\"settings.cat.ssh\":\"SSH\",\"settings.ssh.status\":\"Service status\",\"settings.ssh.enabled\":\"SSH service\",\"settings.ssh.enabled.help\":\"Start/stop the OpenSSH server (sshd). Keep it on to reach this machine from other devices.\",\"settings.ssh.running\":\"running\",\"settings.ssh.stopped\":\"stopped\",";
...
const targets = [
  ...
  },
  {
    ...
    expectOut: 'value.slice(e.selectionStart,e.selectionEnd)',
  ...
  },
  {
    ...
    expectOut: '"settings.download.threads"',
    ...
    transform: c => c
      // #8/#9 重写:revert(先全块态,再现部署局部态)→ apply(全块)。局部 revert 锚点是
      // updates+SSH+APP 连续块(旧 transform 一次链产生的真实部署形态);全块 revert 防御重跑。
