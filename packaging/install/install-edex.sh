...
# Harden against the greeter package's placeholder winning a conf.d merge (seen
# live on a v2.4.25 laptop: lightdm-autologin-greeter.conf ships with
# autologin-user=AUTOLOGIN-USER-NOT-CONFIGURED; when it wins, a lightdm restart
# after an eDEX exit loops "Can't authenticate autologin; autologin not
# configured" -> black screen). Overwrite the package's file with OUR values so
# autologin stays correct regardless of conf.d merge order.
...
# CRITICAL: this unit is written FIRST (before any slow/hang-prone command like
# update-initramfs/update-grub), because it is the single deliverable the whole
# system depends on for autologin. v2.4.22: the script died at update-initramfs
# with this block still at the END — the unit never got written → first boot had
# the autologin placeholder → lightdm crash-loop → black screen. Verify it here
# so a silent write failure is caught at install time, not at first boot.
if [ -s /usr/local/sbin/edex-firstboot.sh ] && [ -f /etc/systemd/system/edex-firstboot.service ]; then
    echo "[edex] first-boot unit OK: edex-firstboot.service written + enabled"
else
    echo "[edex] FATAL: first-boot deliverable missing right after writing — install is broken" >&2
    exit 1
fi
...
# Timeout-bounded: in the curtin chroot these can hang (mount/device probing) —
# a hang here is what cut the v2.4.22 late-commands short, losing the first-boot
# unit that was still at the END of the script. That unit is now written FIRST,
# so even a kill here is survivable; the timeout just stops a hang from stalling
# the whole install (coreutils `timeout` is always present).
timeout 120 update-initramfs -u >/tmp/edex-update-initramfs.log 2>&1 \
    || { echo "[edex] WARN: update-initramfs failed/timed out"; tail -20 /tmp/edex-update-initramfs.log 2>/dev/null; }
timeout 120 update-grub >/tmp/edex-update-grub.log 2>&1 \
    || { echo "[edex] WARN: update-grub failed/timed out"; tail -20 /tmp/edex-update-grub.log 2>/dev/null; }
