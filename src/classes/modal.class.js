...
class Modal {
    constructor(options, onclose) {
        ...
        // UI language: everything except the settings menu is English, so the
        // auto-added close/OK buttons default to English. The settings editor
        // passes `closeLabel: "关闭"` to keep its menu in Chinese.
        let closeLabel = options.closeLabel || "Close";

        switch(this.type) {
            case "error":
                this.classes += " error";
                zindex = 1500;
                buttons.push({label: closeLabel, action:"window.modals['"+this.id+"'].close();"}, {label:"Reload", action:"window.location.reload(true);"});
            ...
            case "custom":
                ...
                buttons.push({label: closeLabel, action:"window.modals['"+this.id+"'].close();"});
