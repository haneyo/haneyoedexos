class Cpuinfo {
    constructor(parentId) {
        ...
        window.si.cpu().then(data => {
            ...
            let cpuName = `${data.manufacturer} ${data.brand}`.trim().substr(0, 30);
    ...
    updateCPUtemp() {
        window.si.cpuTemperature().then(data => {
            try {
                // Apple Silicon does not expose an easily readable thermal sensor
                // (needs root `powermetrics`), so systeminformation returns null there.
                let temp = (data && data.max) ? data.max : null;
                document.getElementById("mod_cpuinfo_temp").innerText = (temp === null) ? "--°C" : `${temp}°C`;
            } catch(e) {
                // See above notice
            }
        });
    ...
    updateCPUspeed() {
        ...
        window.si.cpu().then(data => {
            try {
                let speed = (data.speed && data.speed > 0) ? data.speed : null;
                let speedMax = (data.speedMax && data.speedMax > 0) ? data.speedMax : null;
                document.getElementById("mod_cpuinfo_speed_min").innerText = (speed === null) ? "--GHz" : `${speed}GHz`;
                document.getElementById("mod_cpuinfo_speed_max").innerText = (speedMax === null) ? "--GHz" : `${speedMax}GHz`;
            } catch(e) {
                // See above notice
            }
            this.updatingCPUspeed = false;
