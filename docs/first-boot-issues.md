...
| 32 | 内置下载管理器:接管 Firefox/Chrome 下载,支持网页管理/命令行,设置菜单里自绘 UI 管理 | OS + App | 已修(源码,待装验):内置下载管理器接管浏览器下载 + 设置-下载 分类自绘 UI 管理(#45) |
| 33 | 内置 clash 代理程序(命令行版),浏览器管理 | OS + App | 已修(源码,待装验):内置 mihomo 代理 + 设置-clash 分类 + 全屏浏览器面板(见 ubuntu-side-changes.md §13,#46) |
| 34 | 所有内置程序都要支持更新 | OS + App | 已修(源码,待装验):设置-更新 分类集中管理 App/系统/内置程序(mihomo/Firefox/Claude)(§13,#47) |
...
| 36 | 文件浏览器右键空白处要有"新建文件夹"和"info(属性)"功能 | App | 已修(源码,待装验):文件浏览器右键加 新建文件夹 + info 属性(#49) |
...
**设置输入框复制粘贴丢焦点 / 复制无效**(App,COPY 修复已同步 src+patch;诊断插桩已拆,走新 ISO 验证):
